...
class DecisionTreeClassifier:
    ...
    def __init__(self, eta = 1, pi = 0.99, l = np.inf, p = None, enable_pca_projections = True, enable_lda_projections = True, rs = None, pca_classes = 2, pca_kernel = "linear", project_before_select = False, enforce_projections = False, max_number_of_components_to_consider = 1, allow_global_projection = False, light_weight_split_point = False, lda_on_canonical_projection = True, granularity = 10, beam = None, adjust_lda_via_pca = False):
        self.eta = eta
        self.pi = pi
        self.l = l
        ...
        if self.rotation and X.shape[0] > self.min_instances_for_rotation:
            ...
            if self.enable_pca_projections:
                ...
                if self.allow_global_projection:
                    max_size_of_instances_for_pca_analysis = int(10**8 / X_for_projection.shape[0]) # we can tolerate data matrices of 100 million entries.
                    max_number_of_components_to_consider = min(min(X_for_projection.shape), self.max_number_of_components_to_consider)
                    if self.pca_kernel == "linear":
                        pca = sklearn.decomposition.PCA(n_components = max_number_of_components_to_consider)
                        X_transform = pca.fit_transform(X_for_projection)
                    else:
                        pca = sklearn.decomposition.KernelPCA(n_components = max_number_of_components_to_consider, kernel = self.pca_kernel)
                        if X_for_projection.shape[0] > max_size_of_instances_for_pca_analysis:
                            print(f"Downscaling to {max_size_of_instances_for_pca_analysis} instances for the analysis.")
                            indices = self.rs.choice(range(len(X_for_projection)), max_size_of_instances_for_pca_analysis, replace=False)
                            X_target = X_for_projection[indices]
                        else:
                            X_target = X_for_projection
                    
                        pca.fit(X_target)
                        X_transform = pca.transform(X_for_projection)
                        
                    datasets.append((X_transform, pca, "global pca", None))

                # one PCA to the data reduced to each class
                if self.pca_classes > 0:
                    for k in range(1, self.max_class_combos + 1):
                        ...
                        for labelset in it.combinations([self.labels[l] for l in label_order[-self.pca_classes:]], k):#label_order:#[-self.pca_classes:]:
                            if self.pca_kernel == "linear":
                                ...
                                pca = sklearn.decomposition.KernelPCA(kernel = self.pca_kernel)
...
class RandomForest:
    ...
    def setup(self, X, y):
        self.labels = list(np.unique(y))
        self.trees = []
        self.indices_per_tree = []
        self.X = X
        self.y = y
        
    def get_bootstrap_sample_for_next_tree(self):
        num_instances = self.X.shape[0]
        indices = self.rs.choice(num_instances, num_instances)
        X = np.array([self.X[j] for j in indices])
        y = np.array([self.y[j] for j in indices])
        self.indices_per_tree.append(indices)
        return X, y
        
    def step(self):        
        self.pretransformers_per_tree.append(None)
        X, y = self.get_bootstrap_sample_for_next_tree()
        self.step_on_data(X, y)
        
    def step_on_data(self, X, y):
        p = np.sqrt(X.shape[1]) if self.p is None else self.p
        dt = DecisionTreeClassifier(pi = self.pi, eta = self.eta, p = p, enable_pca_projections = self.enable_pca_projections, enable_lda_projections = self.enable_lda_projections, lda_on_canonical_projection = self.lda_on_canonical_projection, rs = self.rs, project_before_select = self.project_before_select, allow_global_projection = self.allow_global_projection, pca_classes = self.pca_classes, pca_kernel = self.pca_kernel, max_number_of_components_to_consider = self.max_number_of_components_to_consider, enforce_projections = self.enforce_projections, light_weight_split_point = self.light_weight_split_point, granularity = self.granularity, beam = self.beam, adjust_lda_via_pca = self.adjust_lda_via_pca)
        dt.train(X, y)
    ...
    def predict(self, X):
        predictions = []
        opinions = np.array([tree.predict(X) if transformer is None else tree.predict(transformer.transform(X))  for transformer, tree in zip(self.pretransformers_per_tree, self.trees)]).T
    ...
    def get_oob_accuracy(self):
        ...
        # get oob-votes of ensemble
        votes = np.zeros((n, k))
        for i, tree in enumerate(self.trees):
            train_indices = self.indices_per_tree[i]
            mask = np.ones(n).astype(bool)
            mask[train_indices] = False
            X_valid = self.X[mask]
            valid_indices = np.where(mask)[0]
            y_hat = tree.predict(X_valid)
            for index, label in zip(valid_indices, y_hat):
                votes[index, labels.index(label)] += 1
        
        # get oob-predictions of ensemble
        predictions = [labels[i] for i in np.argmax(votes, axis=1)]
        return sklearn.metrics.accuracy_score(self.y, predictions)
    ...
    '''
    def get_cummulative_votes(self, X_test, check = True):
        ...
        votes_valid = [np.zeros((self.X.shape[0], len(self.labels)))]
        votes_test = [np.zeros((X_test.shape[0], len(self.labels)))]

        if len(self.pretransformers_per_tree) != len(self.trees):
            raise Exception(f"Number of transformers and trees does not match. There are {len(self.pretransformers_per_tree)} transformers defined but {len(self.trees)}.")
            
        for i, (transformer, dt) in enumerate(zip(self.pretransformers_per_tree , self.trees)):
            indices_train = set(np.unique(self.indices_per_tree[i]))
            indices_valid = [j for j in range(self.X.shape[0]) if not j in indices_train]
            indices_train = sorted(indices_train)
            X_valid = self.X[indices_valid]
            ...
            if transformer is None:
                ...
                X_valid_tree = transformer.transform(X_valid)
                X_test_tree = transformer.transform(X_test)
            votes_valid_i, votes_test_i = get_votes_of_tree(dt, X_valid_tree, X_test_tree, self.labels)
        ...
        del votes_valid[0]
        del votes_test[0]
        return np.array(votes_valid, dtype=int), np.array(votes_test, dtype=int)

    
class RotationForest(RandomForest):
    
    def __init__(self, n_trees = 100, pi = 0.9, eta = 5, p = None, pca_kernel = "linear", max_number_of_components_to_consider = None, enforce_projections = False, light_weight_split_point = False, granularity = 10, beam = None, timeout = None, rs = None):
        super().__init__(n_trees, pi, eta, p, pca_kernel = pca_kernel, enable_pca_projections = False, enable_lda_projections = False, allow_global_projection = True, project_before_select = False, max_number_of_components_to_consider = max_number_of_components_to_consider, enforce_projections = enforce_projections, light_weight_split_point = light_weight_split_point, granularity = granularity, beam = beam, timeout = timeout, rs = rs)
        
    def step(self):
        
        max_size_of_isntances_for_pca_analysis = int(10**8 / self.X.shape[0]) # we can tolerate data matrices of 100 million entries.
        
        print(f"Computing bootstrap sample")
        X, y = self.get_bootstrap_sample_for_next_tree()
        if self.pca_kernel == "linear":
            pca = sklearn.decomposition.PCA(n_components = min(min(self.X.shape), self.max_number_of_components_to_consider))
            X_transform = X
            
        else:
            if X.shape[0] > max_size_of_isntances_for_pca_analysis:
                print(f"Downscaling to {max_size_of_isntances_for_pca_analysis} instances for the analysis.")
                indices = self.rs.choice(range(len(X)), max_size_of_isntances_for_pca_analysis, replace=False)
                X_transform = X[indices]
            else:
                X_transform = X
            pca = sklearn.decomposition.KernelPCA(n_components = min(min(self.X.shape), self.max_number_of_components_to_consider), kernel = self.pca_kernel)
        
        print(f"Running PCA on {X_transform.shape}-shaped data.")
        pca.fit(X_transform)
        print(f"Ready. Now applying PCA on {X.shape}-shaped data.")
        X = pca.transform(X)
        print(f"Growing tree on {X.shape}-shaped data.")
        self.step_on_data(X, y)
        self.pretransformers_per_tree.append(pca)